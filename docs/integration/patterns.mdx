...
The oracle's 60-second freshness check guarantees the attestation was signed within the last 60 seconds. It does **not** guarantee the attestation was signed inside your consumer's logical time window. For settlement, liquidation, or auction logic, enforce a stronger temporal check in the consumer body:
...
Without that check, an attacker can replay a ≤60 s old attestation to settle an event that was supposed to settle "now".
