...
On-chain the asset is identified by an 8-byte `BytesN<8>` — the first 8 bytes of the signed message. The JS SDK accepts a display name like `"BTC/USD"` and translates internally. A Soroban consumer that validates the tag (Pattern B) needs the bytes directly:
...
`b"BTC/USD\0"` (with the slash) is **not** the on-chain tag. A consumer init'd with that will reject every real attestation as `WrongAsset` and silently lose every settlement. Confirm with the first 8 bytes of `fresh.attestations[0].message`, or read the `tag` field on an `Attestation`.

:::

For the live feed list, call `GET https://api.noeracle.org/v1/latest` and read each entry's `tag` field.
